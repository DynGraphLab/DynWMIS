...
namespace onlinemis {
    ...
    void online_ils::perform_ils(MISConfig &config, graph_access &G,
                                 unsigned int iteration_limit, bool by_unsucc_iterations) {
        ...
        forall_nodes(G, node){
                    if (marked_degree[node] < 0 || marked_degree[node] > 2) { 
                               continue;
                    }
        ...
        unsigned int unsucc_iterations = 0;
        unsigned int prev_best = best_solution;
        while (1) {
            run_t.restart();
            iterations++;

            // Stop if the limits was passed
            if(by_unsucc_iterations) {
                    if( prev_best == best_solution ) {
                            unsucc_iterations++;
                    } else {
                            unsucc_iterations = 0;
            ...
            if (unsucc_iterations > config.unsucc_iterations) { break;}
            plateau--;
            ...
            for (int i = 0, tries = 0; i < config.force_cand; i++, tries++) {
                ...
                if (marked_degree[v] < 0 || (marked_degree[v] > degree_limit && tries < 100) ) {
                    i--;
                    continue;
                }
           ...
           //std::cout <<  "v " <<  v  << std::endl;
            ...
            //if( v >= marked_degree.size() ) {
                //std::cout <<  "problem " << v <<  " " <<  marked_degree.size() << std::endl;
                //exit(0);
            //}
                    ...
                    if (swap_on_failure) {
                        ...
                        if (!one->is_empty()) {
                            ...
                            for (int i = 0; i < config.force_cand; i++) {
                                ...
                                // Modification: Skip nodes with degree higher than the limit
                                // Modification: Don't touch neighbors of marked nodes
                                if (marked_degree[y] < 0 || (marked_degree[y] > degree_limit && tries < 100)) {
                                    i--;
                                    tries++;

...
namespace onlinemis {
    ...
    bool mis_permutation_online::search_adj(NodeID source, NodeID target,
        ...
        int low = G.get_first_edge(source);
        int high = G.get_first_invalid_edge(source) - 1;
        int mid = 0;
        // CS: was EdgeID check what happens TODO
       
        //forall_out_edges(G, e, source) {
                //NodeID target_ = G.getEdgeTarget(e);
                //std::cout <<  source <<  " target " <<  target_ <<  " " <<  target  << std::endl;
        //} endfor
        while (low <= high) {
            mid = low + (high - low) / 2;

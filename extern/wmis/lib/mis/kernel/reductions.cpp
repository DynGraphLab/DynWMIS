...
bool clique_neighborhood_reduction_fast::reduce(branch_and_reduce_algorithm* br_alg) {
    ...
    for_each_changed_vertex(br_alg, [&br_alg, &status](NodeID v) {
        ...
        std::sort(neighbors.begin(), neighbors.end(),
                  [&status](const NodeID lhs, const NodeID rhs) { return status.weights[lhs] > status.weights[rhs]; });
...
bool clique_neighborhood_reduction::partition_into_cliques(NodeID v) {
    ...
    while (neighbors_vec.size() >= 2) {
        ...
        if (expand_clique(max_neighbor, neighbors_vec, clique_neighbors_set)) return true;
    }

    return false;
}

bool clique_neighborhood_reduction::expand_clique(NodeID max_neighbor, sized_vector<NodeID>& neighbors_vec,
                                                  fast_set& clique_neighbors_set) {
    auto& status = br_alg->status;
    ...
    // adjust neigbors_vec
    for (auto neighbor : neighbors_vec) {
        if (!clique_set.get(neighbor)) reamining_neighbors.push_back(neighbor);
    }
...
bool fold2_reduction::reduce(branch_and_reduce_algorithm* br_alg) {
    ...
    for_each_changed_vertex(br_alg, [&](NodeID v) {
        if (br_alg->deg(v) == 2) {
            ...
            for (NodeID u : status.graph[neighbors[0]])
                if (u == neighbors[1]) {
                    // clique of size 3
...
bool clique_reduction::reduce(branch_and_reduce_algorithm* br_alg) {
    ...
    for_each_changed_vertex(br_alg, [&](NodeID node) {
        ...
        // remove all nodes from the clique which have a smaller or eqaul weight than "max_isolated" -> we can always
        // pick "max_isolated" over them
...
void clique_reduction::fold(branch_and_reduce_algorithm* br_alg, const weighted_node& isolated,
                            std::vector<NodeID>&& non_isolated) {
    auto& status = br_alg->status;
...
bool domination_reduction::reduce(branch_and_reduce_algorithm* br_alg) {
    ...
    for_each_changed_vertex(br_alg, [&](NodeID v) {
        ...
        for (NodeID neighbor : status.graph[v]) {
            if (br_alg->deg(neighbor) > neighbors_count) continue;
...
bool generalized_neighborhood_reduction::reduce(branch_and_reduce_algorithm* br_alg) {
    ...
    for_each_changed_vertex(br_alg, [&](NodeID v) {
        ...
        for (NodeID neighbor : status.graph[v]) {
            ...
            if (status.weights[neighbor] > max_neighbor_weight) max_neighbor_weight = status.weights[neighbor];
        }
...
bool generalized_fold_reduction::reduce(branch_and_reduce_algorithm* br_alg) {
    ...
    for_each_changed_vertex(br_alg, [&](NodeID v) {
        ...
        for (const NodeID neighbor : status.graph[v]) {
            if (!MWIS_set.get(neighbor)) continue;
        ...
        do {
            for (const NodeID node : status.graph[v]) {
                ...
                // "force" node into an IS (= remove node and its neighbors from N(v) and compute MWIS in remaining
                // N(v))
                ...
                if (!neighborhood_br_alg.run_branch_reduce()) {
                    ...
                    // if the weight of every MWIS in N(v) which contains "node" is smaller than w(v) then we can remove
                    // "node"
                    remove_node =
                        neighborhood_br_alg.get_current_is_weight() + status.weights[node] <= status.weights[v];
                }
...
void generalized_fold_reduction::fold(branch_and_reduce_algorithm* br_alg, NodeID main_node, fast_set& MWIS_set,
...
bool iterative_struction<struction_type, type, vertex_increase>::reduce(branch_and_reduce_algorithm* br_alg) {
    ...
    bool applied = false;
    for_each_changed_vertex(br_alg, [&](NodeID v) {
        if (br_alg->deg(v) > br_alg->config.struction_degree ||
            !s.reduce(br_alg, v, s.removed_vertices(br_alg, v) + vertex_increase))
            return;
...
bool blow_up_struction<key_function, struction_type, type>::reduce(branch_and_reduce_algorithm* br_alg) {
    ...
    while (!is_done() && clean_up_queue()) {
        ...
        if (f.set_estimate(br_alg, n, key) > plain_set_limit) break;

        queue.deleteMax();
        if (s.reduce(br_alg, n, std::min(set_limit_by_key, plain_set_limit))) {
            // struction was successfully executed
            if (br_alg->config.backtrack_style != MISConfig::Backtrack_Type::IMMEDIATE_EXCLUDE && update_set.add(n))
...
bool blow_up_struction<key_function, struction_type, type>::clean_up_queue() {
    ...
    for (NodeID n : marker.next) update_queue(n);
    marker.clear_next();
    ...
    while (queue.size()) {
        ...
        if (status.node_status[n] == IS_status::not_set && br_alg->deg(n) <= br_alg->config.struction_degree)
            return true;
        if (update_set.add(n)) update_list.emplace_back(n, denoise(queue.maxValue()));
        queue.deleteMax();
    }
    return false;
}
...
template <typename key_function, typename struction_type, reduction_type type>
void blow_up_struction<key_function, struction_type, type>::reset(branch_and_reduce_algorithm* br_alg,
                                                                  size_t comp_size) {
    restored = false;
    queue.clear();
}
...
void blow_up_struction<key_function, struction_type, type>::init_blow_up_phase() {
    ...
    if (restored) {
        for (NodeID n : added_list)
            if (queue.contains(n)) queue.deleteNode(n);
        for (auto& e : update_list) update_queue_by_key(e.first, e.second);
    } else {
        for_each_changed_vertex(br_alg, [&](NodeID n) { update_queue(n); });
    }
...
bool path_reduction::reduce(branch_and_reduce_algorithm* br_alg) {
    ...
    for_each_changed_vertex(br_alg, [&](NodeID v) { enqueue_node<false>(v); });
...
void path_reduction::restore(branch_and_reduce_algorithm* br_alg) {
    ...
    for (auto& n_w : restore_data.node_weights) status.weights[n_w.first] = n_w.second;
    ...
    if(status.use_lower_weight) {
        status.lower_weight += restore_data.offset;
    }
    restore_vec.pop_back();
}
...
bool path_reduction::reduce_degree_one_node() {
    ...
    if (status.weights[n] >= status.weights[u]) {
        ...
        for (NodeID v : status.graph[u]) enqueue_node(v);
    } else {
        restore_vec.emplace_back();
        auto& restore_data = restore_vec.back();
...
bool path_reduction::reduce_path() {
    ...
    if (v == w) {
        ...
        if (br_alg->deg(v) != 2 && w_i > w_e) {
            ...
            status.node_status[v] =
                w_i > w_e ? IS_status::folded : IS_status::excluded;  // set node state to folded instead of included to
                                                                      // be consistent with mis_weight
        ...
        if (connected || w_i_i <= w_i_e || w_i_i <= w_e_i) {
            ...
            if (!connected && keep_v && keep_w) reconnect(v, w, restore_data);
        } else {
    ...
    for (NodeID n : path) restore_vec.back().path.push_back(n);
    return true;
}
...
void path_reduction::add_reduction_offset(size_t offset, restore_data& data) {
    ...
    br_alg->status.decr_lower_weight(offset);
    data.offset = offset;
}
...
void path_reduction::find_MIS_on_path(NodeWeight& w_i, NodeWeight& w_e, sized_vector<NodeID>& path) {
    ...
    for (size_t i = 2; i < path.size(); ++i) {
        if (track_choices) br_alg->bool_buffer[i - 1] = w_i > w_e;
...
bool path_reduction::next_node_on_path(NodeID current, NodeID last, NodeID first, NodeID& next) {
    auto& status = br_alg->status;
    if (br_alg->deg(current) != 2 || current == first) return false;
...
void path_reduction::enqueue_node(NodeID n) {
    if (br_alg->status.node_status[n] != IS_status::not_set) return;
    if (br_alg->deg(n) == 1) br_alg->buffers[0].push_back(n);

...
branch_and_reduce_algorithm::branch_and_reduce_algorithm(graph_access& G, const MISConfig& config,
                                                         bool called_from_fold, size_t init_length_non_impr_seq)
    : config(config),
      length_non_impr_seq(init_length_non_impr_seq),
      global_status(G),
      set_1(global_status.n),
      set_2(global_status.n),
      double_set(global_status.n * 2),
      buffers(4, sized_vector<NodeID>(global_status.n)),
      bool_buffer(global_status.n),
      zero_vec(global_status.n, 0) {
    if (called_from_fold) {
        global_status.transformations = make_reduction_vector<neighborhood_reduction, fold2_reduction, clique_reduction,
...
void branch_and_reduce_algorithm::set_lower_weight(NodeWeight init_weight) {
    // must be called before run_branch_and_reduce
    ASSERT_TRUE(global_status.remaining_nodes == global_status.n);

    global_status.lower_weight = init_weight;
    global_status.use_lower_weight = true;
}
...
void branch_and_reduce_algorithm::set(NodeID node, IS_status mis_status, bool push_modified) {
    ...
    if (push_modified) status.modified_stack.push_back(node);

    if (mis_status == IS_status::included) {
        status.is_weight += status.weights[node];

        status.decr_lower_weight(status.weights[node]);
...
void branch_and_reduce_algorithm::unset(NodeID node, bool restore) {
    if (status.node_status[node] == IS_status::included) {
        ...
        if (status.use_lower_weight) {
            status.lower_weight += status.weights[node];
        }
    }
...
void branch_and_reduce_algorithm::compute_ils_pruning_bound() {
    ...
    if(config.max_length_non_impr_seq < std::numeric_limits<size_t>::max()) {
        // in case branching is limited we use a fixed time limit
        config_cpy.time_limit = 1; // seconds
    }
    config_cpy.time_limit = config_cpy.time_limit * status.n / total_ils_node_count / 100;

    NodeWeight new_best_weight = status.reduction_offset + status.is_weight + run_ils(config_cpy, *local_graph, buffers[0], 1000);
    NodeWeight new_best_is_time = t.elapsed();
    if(new_best_weight > best_weight) {
        is_ils_best_solution = true;

        if ( status.lower_weight != 0 ) {
            is_init_best_solution = false;
            impr_by_comp_lb += new_best_weight-best_weight; // for statistics
        }

        best_weight = new_best_weight;
        best_is_time = new_best_is_time;
    }
...
void branch_and_reduce_algorithm::initial_reduce() {
    ...
    if (!config.blow_up_disabled && status.transformations.size() != status.num_reductions) cyclic_blow_up();
...
bool branch_and_reduce_algorithm::blow_up_graph_internal() {
    bool blown_up = false;
    for (size_t active_blow_up_index = status.num_reductions; active_blow_up_index < status.transformations.size();
         ++active_blow_up_index) {
...
#endif
            phase_count = 0;
        } else if (config.backtrack_style == MISConfig::IMMEDIATE_EXCLUDE ||
                   config.backtrack_style == MISConfig::IMMEDIATE_TIE_BREAKING) {
            ++phase_count;
            undo_blow_up();
        }
    }
    if (config.backtrack_style != MISConfig::NO_BACKTRACK) undo_blow_up();
#ifdef OUTPUT_GRAPH_CONVERGENCE
    std::cout << t.elapsed() << "," << min_kernel << std::endl;
#endif
...
bool branch_and_reduce_algorithm::branch_reduce_recursive() {
    ...
    for (size_t i = 0; i < comp_count; i++) {
        ...
        branch_and_reduce_algorithm br_alg(G, config, true, length_non_impr_seq);
        cout_handler::disable_cout();
        timeout = !br_alg.run_branch_reduce();
        length_non_impr_seq = br_alg.length_non_impr_seq;
        cout_handler::enable_cout();
    ...
    forall_nodes (recursive_graph, node) {
        if (recursive_graph.getPartitionIndex(node) == 1) set<false>(recursive_mapping[node], IS_status::included);
    }
    endfor
...
void branch_and_reduce_algorithm::branch_reduce_single_component() {
    ...
    if(status.use_lower_weight) {
        best_weight = status.lower_weight;
        is_init_best_solution = true;
        is_ils_best_solution = false;
        // we don't use lower_weight as lower bound again
        // because if we branch recursively, status.graph consist of more than one component
        status.use_lower_weight = false;
    }
    ...
    if (status.n > ILS_SIZE_LIMIT && (!is_init_best_solution || config.compute_ils_on_lb_available)) compute_ils_pruning_bound();
    ...
    while (i < status.n) {
        ...
        if(length_non_impr_seq >= config.max_length_non_impr_seq && best_weight != 0) {
            timeout = true;
            break;
        }
        ...
        if (i == status.n - 1) {
            if (status.node_status[branch_node] == IS_status::not_set) set<false>(branch_node, IS_status::included);

            //update_best_solution();
            update_best_solution_at_leaf();
            reverse_branching();
        ...
        bool improvement_possible =
            status.is_weight + status.reduction_offset + compute_cover_pruning_bound() > best_weight;
        if (!improvement_possible) {
            ...
            // still covering case: first branch is not improving
            if (i == 0) {
        ...
        if (old_n < status.n) {
            ...
            std::sort(
                node_order.begin() + old_n, node_order.begin() + status.n, [&](const NodeID lhs, const NodeID rhs) {
                    return deg(lhs) > deg(rhs) || (deg(lhs) == deg(rhs) && status.weights[lhs] > status.weights[rhs]);
                });
...
bool branch_and_reduce_algorithm::run_branch_reduce() {
    ...
    for (size_t i : comp_idx) {
        ...
        // one component, use lower bound on solution weight if a bound exists
        if (comp_idx.size() == 1) {
            if (global_status.use_lower_weight) {
                status.use_lower_weight = true;
                status.lower_weight = global_status.lower_weight;
            }
        }
        ...
        if (is_init_best_solution) {
            // global_status.use_lower_weight && comp_idx.size() == 1 &&
            // only one component which was not improved
            ASSERT_TRUE(status.lower_weight == best_weight);
            return false;
        }else{
            //ASSERT_TRUE(status.is_weight == best_weight);
            ASSERT_TRUE(best_weight > status.lower_weight || best_weight == 0);
        }
    ...
    restore_best_global_solution();
    ASSERT_TRUE(timeout || status.is_weight > status.lower_weight);
    return !timeout;
}
...
void branch_and_reduce_algorithm::update_best_solution() {
    ...
    if (current_weight > best_weight) {
        impr_by_branching += current_weight - best_weight;
...
void branch_and_reduce_algorithm::update_best_solution_at_leaf() {
    NodeWeight current_weight = status.is_weight + status.reduction_offset;
    if (current_weight > best_weight) {
        impr_by_branching += current_weight - best_weight;
        best_solution_status = status;
    ...
    //std::cout << "length non impr seq" << length_non_impr_seq << std::endl;
}

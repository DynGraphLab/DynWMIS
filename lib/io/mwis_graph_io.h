...
class mwis_graph_io {
   public:
    ...
    std::pair<int, int> readEdgeSequence(std::string file,
                                         std::vector<std::pair<int, std::pair<NodeID, NodeID> > >& edge_sequence, std::vector<NodeWeight> &node_weights, bool & has_node_weights);

 };

template <typename vectortype>
void mwis_graph_io::writeVector(std::vector<vectortype>& vec, std::string filename) {
...
template <typename vectortype>
void mwis_graph_io::readVector(std::vector<vectortype>& vec, std::string filename) {
    std::string line;

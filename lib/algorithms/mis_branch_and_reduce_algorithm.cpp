...
int mis_branch_and_reduce_algorithm::REDUCTION = 2;//TODO: for the project reduced to 2, was 3
int mis_branch_and_reduce_algorithm::LOWER_BOUND = 4;
int mis_branch_and_reduce_algorithm::BRANCHING = 2;
bool mis_branch_and_reduce_algorithm::outputLP = false;
long mis_branch_and_reduce_algorithm::nBranchings = 0;
int mis_branch_and_reduce_algorithm::debug = 0;

mis_branch_and_reduce_algorithm::mis_branch_and_reduce_algorithm(vector<vector<int>> &_adj, int const _N)
...
int mis_branch_and_reduce_algorithm::deg(int v) {
    assert(x[v] < 0);
    int deg = 0;
    for (int u : adj[v])
        if (x[u] < 0) deg++;
...
void mis_branch_and_reduce_algorithm::set(int v, int a) {
    assert(x[v] < 0);
    crt += a;
    x[v] = a;
    vRestore[--rn] = v;
...
void mis_branch_and_reduce_algorithm::compute_fold(vector<int> const &S, vector<int> const &NS) {
    ...
    ////    cout << __LINE__ << ", " << this << ", " << depth << ": Setting modifieds[" << modifiedN-1 << "]=" <<
    /// modifieds[modifiedN-1] << endl << flush;
}

void mis_branch_and_reduce_algorithm::compute_alternative(vector<int> const &A, vector<int> const &B) {
    ...
    modifieds[modifiedN++] =
        make_shared<alternative>(alternative(removed.size() / 2, removed, vs, newAdj, this, A2.size()));
    ////    cout << __LINE__ << ", " << this << ", " << depth << ": Setting modifieds[" << modifiedN-1 << "]=" <<
    /// modifieds[modifiedN-1] << endl << flush;
}

void mis_branch_and_reduce_algorithm::restore(int n) {
    while (rn < n) {
        int v = vRestore[rn];
        if (v >= 0) {
            crt -= x[v];
            ...
            ////            cout << __LINE__ << ", " << this << ", " << depth << ": Deleting " << modifieds[modifiedN]
            ///<< endl << flush;
...
void mis_branch_and_reduce_algorithm::reverse() {
...
int mis_branch_and_reduce_algorithm::lpLowerBound() {
    ////    try (Stat stat = new Stat("lb_LP")) {
    return crt + (rn + 1) / 2;
    ////    }
}

int mis_branch_and_reduce_algorithm::cycleLowerBound() {
...
int mis_branch_and_reduce_algorithm::cliqueLowerBound() {
    ////    try (Stat stat = new Stat("lb_clique")) {
    int need = crt;
    vector<long long> ls(rn, 0);
    int k = 0;
    for (int i = 0; i < n; i++)
        if (x[i] < 0) ls[k++] = ((long long)deg(i)) << 32 | i;
    ...
    for (int i = 0; i < rn; i++) {
        ...
        int to = v, max = 0;
        for (int u : adj[v])
            if (x[u] < 0 && used.get(u)) tmp[clique[u]] = 0;
        for (int u : adj[v])
            if (x[u] < 0 && used.get(u)) {
                int c = clique[u];
                tmp[c]++;
...
int mis_branch_and_reduce_algorithm::lowerBound() {
    int type = 0, tmp;
    if (lb < crt) {
        lb = crt;
        type = 1;
...
bool mis_branch_and_reduce_algorithm::dinicDFS(int v) {
    while (iter[v] >= 0) {
...
// helper for lpReduction
void mis_branch_and_reduce_algorithm::updateLP() {
...
bool mis_branch_and_reduce_algorithm::lpReduction() {
    int oldn = rn;
    ////    if (debug >= 3 && depth <= maxDepth) fprintf(stderr, "%sLP:start\n", debugString().c_str());
    updateLP();

////    if (modifiedN >= 5)
////        cout << __LINE__ << ", " << this << ", " << depth << ": Setting modifieds[" << 5 << "]=" << modifieds[5] <<
/// endl << flush; /    if (debug >= 3 && depth <= maxDepth) fprintf(stderr, "%sLP:afterUpdate\n",
/// debugString().c_str());
#if 1
    ...
    for (int s = 0; s < n; s++)
        if (x[s] < 0 && used.add(s)) {
            ...
            while (qt >= 0) {
                int v = que[qt], u = -1;
                ////            if (modifiedN >= 5)
                ////                cout << __LINE__ << ", " << this << ", " << depth << ", s=" << s << ": Setting
                /// modifieds[" << 5 << "]=" << modifieds[5] << endl << flush;
                if (v < n) {
                    ...
                    ////                if (modifiedN >= 5)
                    ////                    cout << __LINE__ << ", " << this << ", " << depth << ", s=" << s << ":
                    /// Setting modifieds[" << 5 << "]=" << modifieds[5] << endl << flush;
                ...
                if (u >= 0) {
                    ...
                    ////                if (modifiedN >= 5)
                    ////                    cout << __LINE__ << ", " << this << ", " << depth << ", s=" << s << ":
                    /// Setting modifieds[" << 5 << "]=" << modifieds[5] << endl << flush; /                cout << ",
                    /// q[" << qt << "]=" << u << endl;
                } else {
                    ////                if (modifiedN >= 5)
                    ////                    cout << __LINE__ << ", " << this << ", " << depth << ", s=" << s << ":
                    /// Setting modifieds[" << 5 << "]=" << modifieds[5] << endl << flush; /                cout << ",
                    /// level[" << p << "/" << level.size() << "]=" << v << " qt=" << qt << endl;
                    level[p++] = v;
                    qt--;
                    ////                if (modifiedN >= 5)
                    ////                    cout << __LINE__ << ", " << this << ", " << depth << ", s=" << s << ":
                    /// Setting modifieds[" << 5 << "]=" << modifieds[5] << endl << flush; /                cout << ",
                    /// level[" << p-1 << "/" << level.size() << "]=" << level[p-1] << ", qt=" << qt << endl;
                }
            }
    ...
    ////        if (modifiedN >= 5)
    ////                cout << __LINE__ << ", " << this << ", " << depth << ": Setting modifieds[" << 5 << "]=" <<
    /// modifieds[5] << endl << flush; /    if (debug >= 3 && depth <= maxDepth) fprintf(stderr, "%sLP:firsthalf\n",
    /// debugString().c_str());
    used.clear();
    for (int i = p - 1; i >= 0; i--)
        if (used.add(level[i])) {
...
////        if (modifiedN >= 5)
////                cout << __LINE__ << ", " << this << ", " << depth << ": Setting modifieds[" << 5 << "]=" <<
/// modifieds[5] << endl << flush; /    }
#endif  // 0
    if (debug >= 3 && depth <= maxDepth && oldn != rn)
        fprintf(stderr, "%sLP: %d -> %d\n", debugString().c_str(), oldn, rn);
    ////    else if (debug >= 3 && depth <= maxDepth) fprintf(stderr, "%sLP: %d -> %d\n", debugString().c_str(), oldn,
    /// rn); /    if (oldn != rn) Stat.count("reduceN_LP", oldn - rn);
    return oldn != rn;
}

bool mis_branch_and_reduce_algorithm::deg1Reduction() {
    ...
    /// rn); /    if (oldn != rn) Stat.count("reduceN_deg1", oldn - rn);
    return oldn != rn;
    ////    }
}

bool mis_branch_and_reduce_algorithm::dominateReduction() {
...
bool mis_branch_and_reduce_algorithm::fold2Reduction() {
...
bool mis_branch_and_reduce_algorithm::twinReduction() {
                        ...
                        if (x[w] < 0 && w != v) {
                            if (p == 1)
                                ...
                                if (p == 3 && deg(w) == 3) {
                                    ...
                                    for (int z : NS)
                                        for (int a : adj[z])
                                            if (x[a] < 0 && vUsed[a] == uid) ind = false;
                                    if (ind) {
...
bool mis_branch_and_reduce_algorithm::funnelReduction() {
    ...
    for (int v = 0; v < n; v++)
        if (x[v] < 0) {
            ...
            if (u1 < 0) {
                ...
                int d1 = 0, d2 = 0;
                for (int w : adj[u1])
                    if (x[w] < 0 && used.get(w)) d1++;
                for (int w : adj[u2])
                    if (x[w] < 0 && used.get(w)) d2++;
...
bool mis_branch_and_reduce_algorithm::deskReduction() {
...
bool mis_branch_and_reduce_algorithm::unconfinedReduction() {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    if (z == -1) {
                        if (REDUCTION >= 3) {
                            ...
                            ////                        cout << __LINE__ << ": packing vector of size " <<
                            /// copyOfqs.size() << endl << flush;
                            packing.emplace_back(std::move(copyOfqs));
                            ////                        cout << __LINE__ << ", " << this << ", " << depth << ":
                            /// packing.size=" << packing.size() << endl << flush;
                        }
                        set(v, 1);
                        goto whileloopend;
                    } else if (z >= 0) {
...
/// oldn, rn); /    if (oldn != rn) Stat.count("reduceN_unconfined", oldn - rn);
#endif  // 0
    return oldn != rn;
    ////    }
}

int mis_branch_and_reduce_algorithm::packingReduction() {
    ...
    for (unsigned int pi = 0; pi < packing.size(); ++pi) {
        ...
        if (a != rn) {
            ...
            for (int j = modifiedN - 1; j >= 0; j--) {
                ////                cout << __LINE__ << ", " << this << ", " << depth << ": modifieds[" << j << "]=" <<
                /// modifieds[j] << endl;
            ...
            for (int j = 0; j < size; j++) {
                for (int u : adj[S[j]])
                    if (x[u] < 0 && count[u] == 1) {
                        ...
                        vector<int> copyOfTmp(tmp.begin(), tmp.begin() + p);
                        ////                    cout << __LINE__ << ": packing vector of size " << copyOfTmp.size() <<
                        /// endl << flush;
                        packing.emplace_back(std::move(copyOfTmp));
                        ////            cout << __LINE__ << ", " << this << ", " << depth << ": packing.size=" <<
                        /// packing.size() << endl << flush;
                    }
            }
            ////        cout << __LINE__ << ": " << endl << flush;
            for (int j = 0; j < size; j++) {
                ...
                if (x[v] < 0 && !used.get(v)) {
                    ...
                    if (sum + p > max) {
                        ...
                        for (int u : adj[v])
                            if (x[u] < 0) qs[q++] = u;
                        vector<int> copyOfqs(qs.begin(), qs.begin() + q);
                        ////                    cout << __LINE__ << ": packing vector of size " << copyOfqs.size() <<
                        /// endl << flush;
                        packing.emplace_back(std::move(copyOfqs));
                        ////            cout << __LINE__ << ", " << this << ", " << depth << ": packing.size=" <<
                        /// packing.size() << endl << flush;
...
/// rn); /        if (oldn != rn) Stat.count("reduceN_packing", oldn - rn);
#endif  // 0
    return oldn != rn ? 1 : 0;
    ////    }
}

void mis_branch_and_reduce_algorithm::branching(timer &t, double time_limit) {
    int oldLB = lb;
    int v = -1, dv = 0;
    ...
    if (REDUCTION >= 3) {
        ...
        packing.emplace_back(std::move(copyOfTmp));
        ////            cout << __LINE__ << ", " << this << ", " << depth << ": packing.size=" << packing.size() << endl
        ///<< flush;
    }
    set(v, 1);
        ...
        if (REDUCTION >= 3) {
            ...
            for (int u : adj[v])
                if (x[u] < 0) {
                    ...
                    vector<int> copyOfTmp(tmp.begin(), tmp.begin() + p);
                    ////                    cout << __LINE__ << ": packing vector of size " << copyOfTmp.size() << endl
                    ///<< flush;
                    packing.emplace_back(std::move(copyOfTmp));
                    ////            cout << __LINE__ << ", " << this << ", " << depth << ": packing.size=" <<
                    /// packing.size() << endl << flush;
...
bool mis_branch_and_reduce_algorithm::decompose(timer &t, double time_limit) {
    ...
    vector<mis_branch_and_reduce_algorithm *> vcs(vss.size(), nullptr);
    {
        for (int i = 0; i < static_cast<int>(vss.size()); i++) {
            ...
            for (int j = 0; j < static_cast<int>(vs.size()); j++) {
                ...
                int p = 0;
                for (int u : adj[vs[j]])
                    if (x[u] < 0) adj2[j][p++] = pos2[u];
                assert(p == adj2[j].size());
                std::sort(adj2[j].begin(), adj2[j].end());
            }

            vcs[i] = new mis_branch_and_reduce_algorithm(adj2, size[i]);
    ...
    }
    {
        for (unsigned int i = 0; i < packingB.size(); i++) {
            ...
            ////            cout << __LINE__ << ", " << this << ", " << depth << ": vcs.packing.size=" <<
            /// vcs[maxID]->packing.size() << endl << flush;
        }
    }
    {
        for (int i = 0; i < modifiedN; i++) {
            ...
            if (p >= 0) {
                ...
                ////                cout << __LINE__ << ", " << this << ", " << depth << ": Setting modifieds[" <<
                /// vcs[p]->modifiedN-1  << "]=" << modifieds[vcs[p]->modifiedN-1] << endl << flush;
            }
        }
    }
    vector<vector<int>> vss2(vss.size());
    {
        ...
        for (int i = 0; i < N; i++)
            if (pos1[i] >= 0) vss2[pos1[i]][pos2[i]] = i;
    }
    int sum = crt;
    for (int i = 0; i < static_cast<int>(vss.size()) && opt > sum; i++) {
        mis_branch_and_reduce_algorithm *vc = vcs[i];
        {
            ...
            for (vector<int> const &ps : vc->packing) {
                ...
                ////            cout << __LINE__ << ", " << this << ", " << depth << ": vc.packing.size=" <<
                /// packing.size() << endl << flush;
            }
        ...
        }
        {
            for (int j = 0; j < vc->modifiedN; j++) {
                ...
                if (dynamic_cast<fold *>(mod.get()) != nullptr) {
                    ...
                    mod = make_shared<alternative>(
                        alternative(removed, vs, this, dynamic_cast<alternative *>(mod.get())->k));
                }
                ////                cout << __LINE__ << ", " << vc << ": Deleting " << vc->modifieds[j] << endl <<
                /// flush; /                delete vc->modifieds[j];
                vc->modifieds[j] = mod;
                ////                cout << __LINE__ << ", " << vc << ": Setting modifieds[" << j << "]=" <<
                /// vc->modifieds[j] << endl << flush;
            }
        }
    ...
    for (mis_branch_and_reduce_algorithm *pAlg : vcs) {
        delete pAlg;
        pAlg = nullptr;
    }
...
bool mis_branch_and_reduce_algorithm::reduce() {
    int oldn = rn;
    for (;;) {
...
void mis_branch_and_reduce_algorithm::rec(timer &t, double time_limit) {
...
int mis_branch_and_reduce_algorithm::solve(timer &t, double time_limit) {
...
std::string mis_branch_and_reduce_algorithm::debugString() const {
    stringstream ins;
#ifdef PUT_TIME
    time_t rawtime;
    struct tm *timeinfo;
...
void mis_branch_and_reduce_algorithm::PrintState() const {
...
void mis_branch_and_reduce_algorithm::reduce_graph() {
    int oldn = rn;
    for (;;) {
        if (REDUCTION >= 0) deg1Reduction();
        ////        if (n > 100 && n * SHRINK >= rn && !outputLP && decompose()) return true;
...
void mis_branch_and_reduce_algorithm::initial_reduce_graph() {
    reduce_graph();
    snapshotX = x;
    reductionSnapshotSize = modifieds.size();
}

size_t mis_branch_and_reduce_algorithm::get_current_is_size() const {
    vector<int> x2(x);
...
size_t mis_branch_and_reduce_algorithm::get_current_is_size_with_folds() const {
...
bool mis_branch_and_reduce_algorithm::folded_vertices_exist() const {
    vector<int> x2(x);
...
vector<int> mis_branch_and_reduce_algorithm::compute_maximal_is() {
...
size_t mis_branch_and_reduce_algorithm::compute_alternative_maximal_is_size() {
    int vertexToForceInIndependentSet(0);
    while (vertexToForceInIndependentSet != -1) {
        reduce_graph();
...
void mis_branch_and_reduce_algorithm::restore_to_snapshot() {
...
size_t mis_branch_and_reduce_algorithm::number_of_nodes_remaining() const {
    size_t node_count(0);
    for (int i : x)
        if (i == -1) node_count++;
...
void mis_branch_and_reduce_algorithm::force_into_independent_set(vector<NodeID> const &nodes) {
...
void mis_branch_and_reduce_algorithm::extend_finer_is(vector<bool> &independent_set) {
...
void mis_branch_and_reduce_algorithm::get_solved_is(vector<bool> &independent_set) {
    for (unsigned int i = 0; i < y.size(); ++i) {
        if (y[i] == 0) independent_set[i] = true;
    }
}

void mis_branch_and_reduce_algorithm::convert_adj_lists(graph_access &G, std::vector<NodeID> &reverse_mapping) const {
    // Number of nodes
